            ...
            <div class="row">
                <div class="col s12 m8">
                    <div class="single-title">
                        <h4 style="color: aliceblue" class="single-title">{{ $property->title }}</h4>
                    </div>

                    <div class="address m-b-30">
                        <i style="color: cyan" class="small material-icons left">place</i>
                        <span style="color: cyan" class="font-18">{{ $property->address }}</span>
                    </div>

                    <div>
                        @if($property->featured == 1)
                            <a  class="btn-floating btn-small disabled"><i style="color: cyan" class="material-icons">star</i></a>
                        @endif
                ...
                <div class="col s12 m4">
                    <div>
                        <h4 style="color: aliceblue" class="left">${{ $property->price }}</h4>
                ...
                <div class="col s12 m8">
                    ...
                    @else
                        <div class="single-image">
                            @if(Storage::disk('public')->exists('property/'.$property->image) && $property->image)
                                <img style="background-color:#282828 " src="{{Storage::url('property/'.$property->image)}}" alt="{{$property->title}}" class="imgresponsive">
                            @endif
                        </div>
                    @endif

                    <div  style="color: aliceblue" class="single-description p-15 m-b-15 border2 border-top-0">
                        {!! $property->description !!}
                    </div>

                    <div >
                        @if($property->features)
                            <ul style="background-color:#282828 " class="collection with-header">
                                <li class="collection-header grey lighten-4"><h5 class="m-0">Features</h5></li>
                                @foreach($property->features as $feature)
                                    <li style="background-color:#282828;color: aliceblue " class="collection-item">{{$feature->name}}</li>
                                @endforeach
                            </ul>
                        @endif
                    </div>
                    ...
                    @if($videoembed)
                        <div class="card-no-box-shadow card">
                            ...
                            <div style="background-color:#282828 " class="card-image center m-t-10">
                                {!! $videoembed !!}
                            </div>
                        </div>
                    @endif
                    ...
                    <div class="card-no-box-shadow card">
                        ...
                        <div style="background-color:#282828 ;color : aliceblue" class="single-narebay p-15">
                            {!! $property->nearby !!}
                        </div>
                    </div>
                        ...
                        <div style="background-color:#282828;color : aliceblue " class="single-narebay p-15">
                                    ...
                                    <div class="comment">
                                        ...
                                        <div class="content">
                                            <div class="author-name">
                                                ...
                                                <span style="color: rgb(209, 200, 200)" class="time">{{ $comment->created_at->diffForHumans() }}</span>

                                                @auth
                                                    <span style="color: rgb(209, 200, 200)" id="commentreplay" class="right replay" data-commentid="{{ $comment->id }}">Replay</span>
                                                @endauth
                                    ...
                                    <div class="comment children">
                                        ...
                                        <div class="content">
                                            <div class="author-name">
                                                ...
                                                <span style="color: rgb(209, 200, 200)" class="time">{{ $commentchildren->created_at->diffForHumans() }}</span>
                                            </div>
                                            <div  class="author-comment">
                            ...
                            @auth
                                <div class="comment-box">
                                    ...
                                    <form action="{{ route('property.comment',$property->id) }}" method="POST">
                                        ...
                                        <textarea style="color: aliceblue" name="body" class="box"></textarea>
                                        <input type="submit"  class="btn cyan" value="Comment">
                                    </form>
                                </div>
                            @endauth
                            ...
                            @guest 
                                <div  class="comment-login">
                                    ...
                                    <a  href="{{ route('login') }}" class="btn cyan">Login</a>
                                </div>
                            @endguest
                    ...
                    <div class="clearfix">
                        ...
                        <div >
                            <ul class="collection with-header m-t-0">
                                ...
                                <li style="background-color:#282828" class="collection-item p-0">
                                    @if($property->user)
                                        <div style="background-color:#282828"  class="card horizontal card-no-shadow">
                                            <div class="card-image p-l-10 agent-image">
                                                <img style="background-color:#282828" src="{{Storage::url('users/'.$property->user->image)}}" alt="{{ $property->user->username }}" class="imgresponsive">
                                            </div>
                                            <div class="card-stacked">
                                                <div class="p-l-10 p-r-10">
                                                    <h5 style="color: aliceblue" class="m-t-b-0">{{ $property->user->name }}</h5>
                                                    <strong style="color: aliceblue">{{ $property->user->email }}</strong>
                                                </div>
                                            </div>
                                        </div>
                                        <div class="p-l-10 p-r-10">
                                            <p style="color: aliceblue">{{ $property->user->about }}</p>
                                            <a style="color: aliceblue" href="{{ route('agents.show',$property->agent_id) }}" class="profile-link">Profile</a>
                                        </div>
                                    @endif
                                </li>
                                ...
                                @foreach($cities as $city)
                                    <li style="background-color:#282828" class="collection-item p-0">
                                        <a class="city-list" href="{{ route('property.city',$city->city_slug) }}">
                                            <span style="color: aliceblue">{{ $city->city }}</span>
                                        </a>
                                    </li>
                                @endforeach
                            </ul>
